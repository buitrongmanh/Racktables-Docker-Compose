...
COPY ./build/RackTables-$RACKTABLES_VERSION.tar.gz RackTables-$RACKTABLES_VERSION.tar.gz
RUN  tar -xzf RackTables-$RACKTABLES_VERSION.tar.gz \
	&& rm RackTables-$RACKTABLES_VERSION.tar.gz \
	&& mv racktables-RackTables-$RACKTABLES_VERSION $RACKTABLES_PATH

ADD ./build/httpd.conf.template /etc/apache2/httpd.conf.template
ADD ./build/docker-entrypoint.sh ./build/make_racktables_secret.php ./build/init_racktables_db.php ./build/utils.php /
